...
hoi4skill clausewitz-reference --game-root "C:\path\Hearts of Iron IV" --mod-path "M:\path\dependency.mod" --output clausewitz_reference.md
...
When `--game-root` is supplied, `prepare-edit-context` also includes indexed game/dependency resources: focus goal sprites, national-spirit idea pictures, event pictures, decision icons, decision-category pictures, leader portraits, country tags, ideologies, effects, and modifiers. It also embeds a Clausewitz syntax reference table for common gameplay intents such as political power, stability, war support, ideas, events, diplomacy, unit creation, state buildings, focus icons, and idea pictures. Use these lists as the verified art/source/syntax catalog before writing. Match icons and portraits by the feature meaning, not by a fixed demo list: ideology families include communist/socialist/soviet, democratic/liberal/parliament/election, fascist/nazi, monarchist/royal/kaiser/tsar, non-aligned/neutral/junta, anarchist/syndicalist; country and region words should bias toward matching tags/names; leader portraits should also match leader role words such as president, chairman, king, emperor, general, marshal, minister, or advisor.

Do not invent effect or modifier names from natural language. If the reference table or indexed game documentation does not verify the primitive, query the Clausewitz code library for a nearby real example or leave a TODO in the structured card/writer layer. Examples of forbidden guessed syntax include `add_opinion = TAG = 20`, `spawn_units = { ... }`, `add_army_org = 0.1`, and `picture = GFX_idea_name` inside an idea block.
