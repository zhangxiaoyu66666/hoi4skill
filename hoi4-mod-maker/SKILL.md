...
- National-focus `icon = ...` values must come from verified focus icon sprites in the target mod, dependency mods, or game `interface/goals*.gfx`; vanilla commonly uses both `GFX_goal...` and `GFX_focus...`. Do not invent icon names from the focus title.
